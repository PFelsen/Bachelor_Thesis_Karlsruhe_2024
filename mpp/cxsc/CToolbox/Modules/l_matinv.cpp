...
/* CVS $Id: l_MatInv.cpp,v 1.14 2014/01/30 17:49:27 cxsc Exp $ */
...
//============================================================================
//----------------------------------------------------------------------------
// File: l_MatInv (implementation)
// Purpose: Computation of an approximate inverse of a l_real square matrix.
// Method: LU decomposition applying Crout's algorithm.
// Global functions:
//   l_MatInv()      : matrix inversion
//   l_MatInvErrMsg(): to get an error message text
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// Purpose: The function 'l_MatInv()' may be used for the computation of an
//    approximate inverse of a l_real square matrix.
// Parameters:
...
void l_MatInv ( l_rmatrix A, l_rmatrix& R, int& Err )
{
  ...
  if (n == 2) {                            // Special case: (2,2)-matrix
    ...
    Temp = mid(l_interval(Accu)); // rnd(Accu);
  ...
  //---------------------------------------
  l_rvector v(n), x(n);          // Help vectors
  ...
  while ( (Err == NoError) && (i <= n) ) {
    ...
    for (k = i; k <= n; k++) {   // v_k = #*(A_ki - sum(j=1(1)i-1,A_kj*A_ji))
      ...
      v[k] = mid(l_interval(Accu)); //rnd(Accu);
    }
    ...
    for (k = i+1; k <= n; k++) { // A_ik = #*(A_ik - sum(j=1(1)i-1,A_ij*A_jk))
      ...
      A[i][k] = mid(l_interval(Accu)); //rnd(Accu);
      A[k][i] = v[k] / Temp;
    }
    i++;
  } // while
  ...
  for (k = 1; k <= n; k++) {
    ...
    for (i = l+1; i <= n; i++) {    // x_i = - #*(sum(j=1(1)i-1,A_ij*x_j))
      ...
      x[i] = -mid(l_interval(Accu)); //-rnd(Accu);
    }
    ...
    for (i = n; i >= 1; i--) {   // x_i = #*(x_i-sum(j=i+1(1)n,A_ij*x_j)/A_ii)
      ...
      x[i] = mid(l_interval(Accu)) / A[i][i]; //rnd(Accu) / A[i][i];
