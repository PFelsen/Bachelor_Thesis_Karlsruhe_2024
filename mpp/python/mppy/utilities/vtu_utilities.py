...
try:
    ...
    def vtkXMLGenericDataObjectReader():
        print("vtu module was not found")
...
class VtuSingleReader(VtuReader):
...
class VtuScalarReader(VtuSingleReader):
    def __init__(self, filename, cwd=os.getcwd(), convert_to_cell_data=False):
        ...
        # What if cell and point data are both set?
        # This is botchery but I don't have enough time to completly rewrite this
        # and frankly I shouldn't need to
        if cell_data_scalars is not None and self.values is None:
            self.has_cell_data = True
            self.values = vtk_to_numpy(cell_data_scalars)


class VtuVectorReader(VtuSingleReader):
...
class VtuGroupScalarReader(VtuReader):
    def __init__(self, group, cwd=os.getcwd()):
        ...
        for i in range(n_sol):
            self.reader.SetFileName(self.wd + group + "%04d.pvtu" % i)
            self.reader.Update()
...
class VtuPlot:
    ...
    def add_pcolormesh(self, file, cb=True, ax=None, **kwargs):
        if ax is None:
            ax = self.ax
        solution = VtuScalarReader(file, self.wd)
    ...
    def add_quivers(self, file, quiver_filter=2, quiver_scale=0.15, ax=None, **kwargs):
        vec_reader = VtuVectorReader(file, self.wd)
    ...
    def add_vtu(self, file, add_mesh=False, mesh_linewidth=0.1, cb=True, ax=None,
        ...
        if ax is None:
            ax = self.ax
        scalar_reader = VtuScalarReader(file, self.wd)
        vec_reader = VtuVectorReader(file, self.wd)
        ...
        if scalar_reader.has_cell_data:
            self.add_patch_collection(scalar_reader, cb=cb, ax=ax, vmin=vmin, vmax=vmax,
                                      **kwargs)
            if add_mesh:
                self.add_mesh('u.pvtu', point_data=False, linewidth=mesh_linewidth, ax=ax)
        if scalar_reader.has_point_data:
            self.add_point_data_patch_collection(scalar_reader, cb=cb, ax=ax, vmin=vmin,
                                                 vmax=vmax,
                                                 **kwargs)
            if add_mesh:
                self.add_mesh('u.pvtu', point_data=True, linewidth=mesh_linewidth, ax=ax)
