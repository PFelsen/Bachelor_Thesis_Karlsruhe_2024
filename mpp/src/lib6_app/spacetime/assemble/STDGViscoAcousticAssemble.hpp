#ifndef _DGVISCOACUSTICDGT_H_
#define _DGVISCOACUSTICDGT_H_
...
class STDGViscoAcousticAssemble : public STAssemble {
protected:
...
public:
  ...
  void PrintInfo() const override {
    ...
    mout.PrintInfo("Assemble", verbose,
                   ...
                   PrintInfoEntry("PG(weighted Testfunc)", WeightedAssemble, 1));
  }

  STDGViscoAcousticAssemble(const Meshes &meshes, DegreePair degree, const string &problemName) :
      STDGViscoAcousticAssemble(meshes, degree, CreateAcousticProblemShared(problemName)
      ) {}

  SeismogramData measure(const ObservationSpecification &specification, const Vector &u) const override;

  ~STDGViscoAcousticAssemble() override = default;
  ...
  void PlotParameters(const Vector &u, std::string suffix) const override;

  const char *Name() const { return "STDGViscoAcousticAssemble"; }
  ...
  void PX_P0(const vector<Scalar> &F,
             ...
             const int &f_deg,
             const int &c_deg,
             const VectorMatrixBase &u) const {
    ...
    for (int k = 0; k < prob->Dim(); ++k)
      for (int i = 0; i < np.size(); ++i)
        for (int j = 0; j < u.GetDoF().NodalPointsLocal(f_deg); ++j)
          C[k * np.size() + i] +=
              F[k * u.GetDoF().NodalPointsLocal(f_deg) + j] * PX_space(np[i], j);
  }
  ...
  void RHS(Vector &) const override;

  void Energy(const Vector &u, vector<double> &Energy) const override;
  ...
  bool HasConformingLagrangeInterpolation() {
    return true;
  }

  void
  ConformingLagrangeInterpolation(const Vector &u, Vector &conf, int degree = 1) const override;
  ...
  virtual void PlotDualSolution(const Vector &dual_u, const Vector &eta, int run) override;

  Vector CalculateMaterialUpdate(const Vector &material, const Vector &forwardSolution, const Vector &backwardSolution);

};

inline Scalar VFlux_c(const VectorField &V, const VectorField &TN) {
  Scalar TNV = TN * V;
  return TNV;
}

inline Scalar VFlux_cf(const VectorField &V, const VectorField &TN, bool bnd,
                int bnd_id = 2) {
  Scalar TNV = TN * V;
  if (!bnd)
    return TNV;
...
inline Scalar PFlux_cf(const double &P, bool bnd, int bnd_id = 2) {
  if (!bnd)
    return P;
  else {
