...
STDGDGViscoAcousticFaceElementGLGL::STDGDGViscoAcousticFaceElementGLGL
...
const Point &STDGDGViscoAcousticFaceElementGLGL::QLocal(int q) const { return qLocal[q]; }

const Point &STDGDGViscoAcousticFaceElementGLGL::QPoint(int q) const { return qPoint[q]; }

const Point &STDGDGViscoAcousticFaceElementGLGL::QNormal(int q) const { return qNormal[q]; }

double STDGDGViscoAcousticFaceElementGLGL::QWeight(int q) const { return qWeight[q]; }

int STDGDGViscoAcousticFaceElementGLGL::nSpaceQ() const { return SpaceQ.size(); }

int STDGDGViscoAcousticFaceElementGLGL::nTimeQ() const { return TimeQ.size(); }

int STDGDGViscoAcousticFaceElementGLGL::nQ() const { return SpaceQ.size() * TimeQ.size(); }

bool STDGDGViscoAcousticFaceElementGLGL::is_zero_test(int nq, int i) const { return i_zero[nq * i_dimension() + i]; }

bool STDGDGViscoAcousticFaceElementGLGL::is_zero(int nq, int j) const { return j_zero[nq * j_dimension() + j]; }

int STDGDGViscoAcousticFaceElementGLGL::i_dimension() const { return shape.size() * (1 + dim + numL); }

int STDGDGViscoAcousticFaceElementGLGL::j_dimension() const { return shape.size() * (1 + dim + numL); }

int STDGDGViscoAcousticFaceElementGLGL::variable(int j) const {
  const Shape &SS = shape.GetSpaceShape();
...
VectorField STDGDGViscoAcousticFaceElementGLGL::Velocity(int nq, int j) {
  return velocityST[nq * j_dimension() + j];
}

double STDGDGViscoAcousticFaceElementGLGL::Pressure(int nq, int j) {
  return pressureST[nq * j_dimension() + j];
}

double STDGDGViscoAcousticFaceElementGLGL::Pressure(int nq, const double *u) {
  double P = 0;
  for (int j = 0; j < j_dimension(); ++j)
    P += u[j] * Pressure(nq, j);
  return P;
...
VectorField STDGDGViscoAcousticFaceElementGLGL::Velocity(Point QP, int i) {
  int var = variable(i);
...
VectorField STDGDGViscoAcousticFaceElementGLGL::Velocity(int nq, const double *u) {
  VectorField V = zero;
  for (int j = 0; j < j_dimension(); ++j)
    V += u[j] * Velocity(nq, j);
  return V;
...
double STDGDGViscoAcousticFaceElementGLGL::Pressure(Point QP, int i) {
  int var = variable(i);
  if (var == 0) return 0.0;
  const Shape &SS = shape.GetSpaceShape();
  const Shape &TS = shape.GetTimeShape();
