...
enum class DataSetType : int { PointData = 0, CellData = 1 };

using DataSetInfo = std::tuple<std::string, DataSetType, size_t>;
using DataSetData = std::vector<float>;
...
struct AsciiWriter {
  template<typename T>
  void writeData(std::ostream &output, const std::vector<T> &data);
...
#define VTU11_CHECK(expr, message)                                                                 \
  if (!(expr)) VTU11_THROW(message)
...
public:
  ScopedXmlTag(std::ostream &output, const std::string &name, const StringStringMap &attributes);
...
void writeEmptyTag(std::ostream &output, const std::string &name,
                   const StringStringMap &attributes);

template<typename T>
inline std::string appendSizeInBits(const char *str) {
...
inline typename std::enable_if<
    std::numeric_limits<T>::is_integer && !std::numeric_limits<T>::is_signed, std::string>::type
dataTypeString() {
...
template<typename T>
inline typename std::enable_if<std::is_same<T, double>::value || std::is_same<T, float>::value,
                               std::string>::type
dataTypeString() {
...
struct Vtu11UnstructuredMesh {
  const std::uint_fast8_t pointDim_ = 3;
  ...
  size_t numberOfPoints() { return points_.size() / pointDim_; }
...
void writeVtu(const std::string &filename, MeshGenerator &mesh,
              ...
              const StringStringMap &pointMap = {}, const StringStringMap &cellMap = {});

//! Creates path/baseName.pvtu and path/baseName directory
void writePVtu(const std::string &path, const std::string &baseName,
               const std::vector<DataSetInfo> &dataSetInfo, size_t numberOfFiles,
               const StringStringMap &pointMap = {}, const StringStringMap &cellMap = {});

//! Forwards path/baseName.vtu to the writeVtu function
template<typename MeshGenerator>
void writePartition(const std::string &path, const std::string &baseName, MeshGenerator &mesh,
                    const std::vector<DataSetInfo> &dataSetInfo,
                    const std::vector<DataSetData> &dataSetData, size_t fileId,
                    const std::string &writeMode = "RawBinaryCompressed");
...
inline void writeTag(std::ostream &output, const std::string &name,
                     const StringStringMap &attributes, const std::string &tagEnd) {
  output << "<" << name;
...
inline ScopedXmlTag::~ScopedXmlTag() { closeTag(); }

inline void writeEmptyTag(std::ostream &output, const std::string &name,
                          const StringStringMap &attributes) {
  detail::writeTag(output, name, attributes, "/>");
}
...
inline std::string base64Encode(Iterator begin, Iterator end) {
  ...
  if (it != end) {
    ...
    size_t remainder =
        static_cast<size_t>(std::distance(it, end)) * size - static_cast<size_t>(byteIndex);
...
template<typename T>
inline void AsciiWriter::writeData(std::ostream &output, const std::vector<T> &data) {
  char buffer[64];
...
template<>
inline void AsciiWriter::writeData(std::ostream &output, const std::vector<std::int8_t> &data) {
  for (auto value : data) {
...
inline StringStringMap AsciiWriter::appendedAttributes() { return {}; }

// ----------------------------------------------------------------

template<typename T>
inline void Base64BinaryWriter::writeData(std::ostream &output, const std::vector<T> &data) {
...
inline StringStringMap Base64BinaryWriter::appendedAttributes() { return {}; }

// ----------------------------------------------------------------

template<typename T>
inline void Base64BinaryAppendedWriter::writeData(std::ostream &, const std::vector<T> &data) {
...
template<typename T>
inline void RawBinaryAppendedWriter::writeData(std::ostream &, const std::vector<T> &data) {
  HeaderType rawBytes = data.size() * sizeof(T);
...
// Somewhere in vtu11/inc/filesystem.hpp, with MSVC, max is defined as macro.
// This pre- vents us from using std::numeric_limits<T>::max( ), so we turn off
// these checks.
#ifndef max
...
inline StringStringMap writeDataSetHeader(Writer &&writer, const std::string &name,
                                          size_t ncomponents) {
  StringStringMap attributes = {{"type", dataTypeString<DataType>()}};

  if (name != "") { attributes["Name"] = name; }

  if (ncomponents >= 1) { attributes["NumberOfComponents"] = std::to_string(ncomponents); }
...
inline void writeDataSet(Writer &writer, std::ostream &output, const std::string &name,
                         size_t ncomponents, const std::vector<DataType> &data) {
...
inline void writeDataSets(const std::vector<DataSetInfo> &dataSetInfo,
                          const std::vector<DataSetData> &dataSetData, std::ostream &output,
                          Writer &writer, DataSetType type) {
  for (size_t iDataset = 0; iDataset < dataSetInfo.size(); ++iDataset) {
    ...
    if (std::get<1>(metadata) == type) {
      detail::writeDataSet(writer, output, std::get<0>(metadata), std::get<2>(metadata),
...
template<typename Writer>
inline void writeDataSetPVtuHeaders(const std::vector<DataSetInfo> &dataSetInfo,
  ...
  for (size_t iDataset = 0; iDataset < dataSetInfo.size(); ++iDataset) {
    ...
    if (std::get<1>(metadata) == type) {
      auto attributes =
          detail::writeDataSetHeader<float>(writer, std::get<0>(metadata), std::get<2>(metadata));
...
inline void writeVTUFile(const std::string &filename, const char *type, Writer &&writer,
...
inline void writeVtu(const std::string &filename, MeshGenerator &mesh,
                     const std::vector<DataSetInfo> &dataSetInfo,
                     const std::vector<DataSetData> &dataSetData, const StringStringMap &pointMap,
                     const StringStringMap &cellMap, Writer &&writer) {
  detail::writeVTUFile(filename, "UnstructuredGrid", writer, [&](std::ostream &output) {
    {
      ScopedXmlTag unstructuredGridFileTag(output, "UnstructuredGrid", {});
      {
        ...
        {
          ScopedXmlTag pointDataTag(output, "PointData", pointMap);

          detail::writeDataSets(dataSetInfo, dataSetData, output, writer, DataSetType::PointData);

        } // PointData

        {
          ScopedXmlTag cellDataTag(output, "CellData", cellMap);

          detail::writeDataSets(dataSetInfo, dataSetData, output, writer, DataSetType::CellData);
...

        {
          ...
          detail::writeDataSet(writer, output, "", mesh.pointDimension(), mesh.points());
...
inline void writeVtu(const std::string &filename, MeshGenerator &mesh,
                     const std::vector<DataSetInfo> &dataSetInfo,
                     const std::vector<DataSetData> &dataSetData, const std::string &writeMode,
                     const StringStringMap &pointMap, const StringStringMap &cellMap) {
  auto mode = writeMode;
  ...
  if (mode == "ascii") {
    detail::writeVtu(filename, mesh, dataSetInfo, dataSetData, pointMap, cellMap, AsciiWriter{});
  } else if (mode == "base64inline") {
    detail::writeVtu(filename, mesh, dataSetInfo, dataSetData, pointMap, cellMap,
                     Base64BinaryWriter{});
...
#ifdef VTU11_ENABLE_ZLIB
    detail::writeVtu(filename, mesh, dataSetInfo, dataSetData, pointMap, cellMap,
                     CompressedRawBinaryAppendedWriter{});
#else
    detail::writeVtu(filename, mesh, dataSetInfo, dataSetData, pointMap, cellMap,
                     RawBinaryAppendedWriter{});
#endif
...
template<typename MeshGenerator>
inline void writePVtu(const std::string &path, const std::string &baseName,
                      const MeshGenerator &mesh, const std::vector<DataSetInfo> &dataSetInfo,
                      const size_t numberOfFiles, const StringStringMap &pointMap = {},
                      const StringStringMap &cellMap = {}) {
  ...
  if (!vtu11fs::exists(directory)) { vtu11fs::create_directories(directory); }

  detail::PVtuDummyWriter writer;

  detail::writeVTUFile(pvtufile.string(), "PUnstructuredGrid", writer, [&](std::ostream &output) {
    std::string ghostLevel = "0"; // Hardcoded to be 0

    ScopedXmlTag pUnstructuredGridFileTag(output, "PUnstructuredGrid",
                                          {{"GhostLevel", ghostLevel}});

    {
      ScopedXmlTag pPointDataTag(output, "PPointData", pointMap);
    ...
    {
      ScopedXmlTag pCellDataTag(output, "PCellData", cellMap);
...

    {
      ...
      StringStringMap attributes = {{"type", dataTypeString<double>()},
                                    {"NumberOfComponents", std::to_string(mesh.pointDimension())}};
...
template<typename MeshGenerator>
inline void writePartition(const std::string &path, const std::string &baseName,
                           MeshGenerator &mesh, const std::vector<DataSetInfo> &dataSetInfo,
                           const std::vector<DataSetData> &dataSetData, size_t fileId,
                           const std::string &writeMode) {
  auto vtuname = baseName + "_" + std::to_string(fileId) + ".vtu";

  auto fullname = vtu11fs::path{path} / vtu11fs::path{baseName} / vtu11fs::path{vtuname};
